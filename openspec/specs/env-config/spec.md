...
When `NEXUS_TERMUX_ACCESSIBLE=true`, MUST prepend `TERMUX_BIN` and `TERMUX_PREFIX/local/bin` to PATH. Each dir guarded by `[ -d "$dir" ]`. Re-source idempotent via `case ":$PATH:"` guard — no duplicate entries, no error.
