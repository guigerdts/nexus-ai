...
func patchDetectGo(src string) (string, error) {
	...
	// Patch 1: IsSupportedOS — add android
	...
	// Patch 2: resolvePlatformProfile — insert case android before default
	old2 := tab(1) + `case "windows":` + "\n" +
		tab(2) + `profile.PackageManager = "winget"` + "\n" +
		tab(2) + `profile.Supported = true` + "\n" +
		tab(2) + `return profile` + "\n" +
		tab(1) + `default:`
	new2 := tab(1) + `case "windows":` + "\n" +
		tab(2) + `profile.PackageManager = "winget"` + "\n" +
		tab(2) + `profile.Supported = true` + "\n" +
		tab(2) + `return profile` + "\n" +
		tab(1) + `case "android":` + "\n" +
	...
	// Patch 3: osReleaseContent — read from PREFIX/etc/os-release for android
	old3 := `func osReleaseContent(goos string) (string, error) {` + "\n" +
		tab(1) + `if goos != "linux" {` + "\n" +
		tab(2) + `return "", nil` + "\n" +
		tab(1) + `}`
	new3 := `func osReleaseContent(goos string) (string, error) {` + "\n" +
...
func patchEngramDownloadGo(src string) (string, error) {
	...
	// Patch 1: redirect goos android to linux when downloading
	old1 := tab(1) + `goos := profile.OS` + "\n" +
		tab(1) + `goarch := normalizeArch(runtime.GOARCH)`
	new1 := tab(1) + `goos := profile.OS` + "\n" +
		tab(1) + `if goos == "android" {` + "\n" +
		tab(2) + `goos = "linux"` + "\n" +
		tab(1) + `}` + "\n" +
		tab(1) + `goarch := normalizeArch(runtime.GOARCH)`
	...
	// Patch 2: engramInstallDir — use PREFIX/bin for android
	old2 := `func engramInstallDir(goos string) string {` + "\n" +
		tab(1) + `if goos == "windows" {`
	new2 := `func engramInstallDir(goos string) string {` + "\n" +
		tab(1) + `if goos == "android" {` + "\n" +
		tab(2) + `prefix := os.Getenv("PREFIX")` + "\n" +
		tab(2) + `if prefix != "" {` + "\n" +
