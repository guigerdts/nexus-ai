...
The system MUST export `NEXUS_VERSION="0.1.0"`, `NEXUS_LANG="es"`, color escape codes in cyan (`NEXUS_COLOR_PRIMARY`) and reset (`NEXUS_COLOR_RESET`), `NEXUS_AGENTS_DIR` pointing to `modules/`, and `NEXUS_LOG_FILE` pointing to `logs/nexus.log`.

#### Scenario: All variables defined after sourcing

- GIVEN `config/env.sh` is sourced
- WHEN checking the environment
- THEN `NEXUS_VERSION` MUST be `"0.1.0"`
